...
void solve() {
  ll n, k; cin >> n >> k;
  map<ll, ll, greater<ll>> mod_count;
  ll max_count = 0, rem;
  vll max_mod;
  vll a(n);
  frange(i, 0, n) {
    ...
    rem = (k - a[i]%k) % k;
    mod_count[rem]++;
    if (rem && mod_count[rem] > max_count) max_count = mod_count[rem];
  }

  if (max_count == 0) cout << 0 << endl;
  else {
    for (auto [key, v] : mod_count) {
      if (v == max_count) {
        cout << k * (max_count-1) + key + 1 << endl;
        break;
      }
    }
  }
