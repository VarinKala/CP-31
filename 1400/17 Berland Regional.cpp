...
void solve() {
  ll n; cin >> n;
  vll u(n); frange(i, 0, n) cin >> u[i];
  map<ll, vll> uni_scores;
  ll s;
  frange(i, 0, n) {
    cin >> s;
    uni_scores[u[i]].pb(s);
  ...
  vll ans(n);
  for (auto [k, v] : uni_scores) {
    sort(all(v), greater<ll>());
    uni_scores[k] = v;
    frange(i, 1, (ll)v.size()) uni_scores[k][i] += uni_scores[k][i-1];
    frange(i, 1, (ll)v.size()+1) ans[i-1] += uni_scores[k][(v.size() / i) * i - 1];
  }
