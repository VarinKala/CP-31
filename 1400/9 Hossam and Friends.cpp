...
void solve() {
  ...
  ll ans = 1, length = 1, left = 2;
  map<ll, ll> cur;
  for (auto el : stranger[1]) cur[el] = 1;
  while (left <= n) {
    if (!cur.count(left) || cur[left] < left - length) length++;
    else length = left - cur[left];
    ans += length;
    for (auto el : stranger[left]) cur[el] = left;
    left++;
  }
