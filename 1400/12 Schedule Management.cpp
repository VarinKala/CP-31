...
void solve() {
  ...
  ll day = 1, left = 0, right = m-1, working = 0, length, rem;
  while (left <= right) {
    length = 0;
    while (left + length < min(m, left + n - working) && a[left + length] == day) length++;
    working = rem = min(right - left - length + 1, n - working - length);
    right -= rem;
    left += length;
    day++;
  }
