...
void solve() {
  ...
  vll gaps(m+1);
  vll suffix(m+2, 0);
  gaps[m] = suffix[m] = n - 1 - neg[m-1];
  frev(i, m, 1) {
    gaps[i] = neg[i] - 1 - neg[i-1];
    suffix[i] = suffix[i+2] + gaps[i];
  }
  gaps[0] = neg[0];
  suffix[0] = suffix[2] + neg[0];
  
  ll sumn = 0, sump = 0;
  frange(i, 0, m) {
    sumn += (gaps[i]+1) * (suffix[i+1] + (m - i + 1) / 2);
    sump += (gaps[i] * (gaps[i] + 1)) / 2;
    if (i < m-1) sump += (gaps[i]+1) * (suffix[i+2] + (m - i) / 2);
  }
  sump += (gaps[m] * (gaps[m] + 1)) / 2;
