...
void solve() {
  ...
  vector<vll> dp(2, vll(n+1, LLONG_MAX));
  dp[0][n] = 0;
  dp[1][n] = 0;

  ll idx = n-1;
  while (idx >= 0) {
    if (a[idx] == 1LL) dp[0][idx] = min(dp[0][idx], dp[1][idx+1] + 1);
    else {
      dp[0][idx] = min(dp[0][idx], dp[1][idx+1]);
      if (idx > 0 && a[idx-1] == 1LL) dp[0][idx-1] = min(dp[0][idx-1], dp[1][idx+1] + 1);
      else if (idx > 0) dp[0][idx-1] = min(dp[0][idx-1], dp[1][idx+1]);
    }

    dp[1][idx] = min(dp[1][idx], dp[0][idx+1]);
    if (idx > 0) dp[1][idx-1] = min(dp[1][idx-1], dp[0][idx+1]);
