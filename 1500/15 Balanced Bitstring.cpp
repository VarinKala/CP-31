...
void solve() {
  ...
  ll temp = count['?'] - max(count['1'], count['0']) + min(count['1'], count['0']);
  if (temp < 0 || temp % 2) {
    cout << "NO" << endl;
    return;
  }
  ...
  while (idx < n) {
    if (s[idx] != '?' && s[idx-k] != '?' && s[idx] != s[idx-k]) {
      cout << "NO" << endl;
      return;
    }

    if (s[idx] == '?' && s[idx-k] == '?') idx = idx;
    else if (s[idx-k] == '?') {
      count['?']--;
      count[s[idx]]++;
    } else if (s[idx] == '?') s[idx] = s[idx-k];

    temp = count['?'] - max(count['1'], count['0']) + min(count['1'], count['0']);
    if (temp < 0 || temp % 2) {
      cout << "NO" << endl;
      return;
    }
