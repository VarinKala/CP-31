...
ll calcSum(ll i, ll sum, ll idx, ll n) {
  if (i == power.size()) {
    min_len = min(min_len, idx + __builtin_popcountll(n-sum));
    if (min_len == 1) return 1;
    return 0;
  }
  
  if (calcSum(i+1, sum, idx, n)) return 1;
  if (sum + power[i] <= n && calcSum(i+1, sum + power[i], idx+1, n)) return 1;
  return 0;
}
...
void solve() {
  ...
  min_len = __builtin_popcountll(n);

  if (min_len > 1) calcSum(0, 0, 0, n);
  cout << min_len << endl;
}
