...
pair<ll, ll> dfs(vector<vll>& edges, vll& sz, vll& parent, ll v) {
  for (auto cur : edges[v]) {
    if (parent[cur] == 0) {
      parent[cur] = v;
      sz[v] += 1 + dfs(edges, sz, parent, cur).S;
    }
  }
...
void solve() {
  ...
  parent[1] = 1;
  pair<ll, ll> c = dfs(edges, sz, parent, 1);
