...
void solve() {
  ...
  ll right;
  frange(i, 0, n) {
    right = upper_bound(all(prefix), prefix[i] + a[i]) - prefix.begin() - 1;
    count[i]++;
    if (right < n) {
      count[right]--;
      ans[right] += prefix[i] + a[i] - prefix[right];
    }
  }

  frange(i, 1, n) count[i] += count[i-1];
  frange(i, 0, n) ans[i] += count[i] * b[i];
