...
void solve() {
  ...
  vll prefix(n+1, 0);
  sort(all(a));
  frange(i, 0, n) prefix[i+1] = prefix[i] + a[i];

  ll right = n-1, covered = 0, ops = 0, left = 0;
  while (left <= right) {
    while (left < right && prefix[left+1] < covered + a[right]) left++;
    if (left < right) {
      ops += a[right] + 1;
      if (prefix[left+1] == covered + a[right]) left++;
      covered += a[right];
      right--;
    } else if (prefix[n] - 2*covered == 1) {
      ops += 1;
      break;
    } else {
      ops += (prefix[n] - 2*covered) - (prefix[n] - 2*covered) / 2 + 1;
      break;
    }
  }
