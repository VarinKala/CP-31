...
void solve() {
  ...
  frange(i, 0, n-1) {
    cin >> u >> v >> x;
    if (!x) {
      if (DSU_parent[u] == -1) DSU_make(u);
      if (DSU_parent[v] == -1) DSU_make(v);
      DSU_union(u, v);
    }
  }
  ...
  map<ll, ll> count;
  frange(i, 1, n+1) {
    if (DSU_parent[i] == -1) DSU_make(i);
    count[DSU_parent[i]] = DSU_size[DSU_find(i)];
  }
  
  ll total = mod_pow(n, k, MOD), cur;
  for (auto [key, v] : count) {
    cur = mod_pow(v, k, MOD);
    if (cur > total) total = (total + (MOD - cur)) % MOD;
    else total -= cur;
  }
