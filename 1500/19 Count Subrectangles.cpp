...
void solve() {
  ...
  frev(i, m-1, 0) prefixb[i] += prefixb[i+1];

  ll ans = 0, val = 0, width = 0, height;
  frange(i, 0, n) {
    if (a[i]) width++;
    else {
      width = 0;
    ...
    height = -1;
    if (k % width == 0 && k > 0) height = k / width;
    
    if (height == -1) ans += val;
    else {
      if (height <= m) val += prefixb[height-1];
      ans += val;
    }
  }
