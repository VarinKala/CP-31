...
void solve() {
  ...
  vll gaps(n, LLONG_MAX);
  ll cur_gap;
  for (auto [k, v] : occurs) {
    cur_gap = v[0];
    frange(i, 1, v.size()) cur_gap = max(cur_gap, v[i] - v[i-1]);
    cur_gap = max(cur_gap, n + 1 - v[v.size()-1]);
    gaps[cur_gap-1] = min(gaps[cur_gap-1], k);
  }

  ll min_ans = LLONG_MAX;
  frange(i, 0, n) {
    min_ans = min(min_ans, gaps[i]);
    if (min_ans == LLONG_MAX) cout << -1 << " ";
    else cout << min_ans << " ";
  }
  cout << endl;
}
