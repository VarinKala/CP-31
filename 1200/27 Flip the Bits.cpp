...
void solve() {
  ...
  ll start, parity, count0 = 0, count1 = 0, left = 0, count = 0;
  while (left < n) {
    if (count0 == count1) start = 1;
    if (start) {
      count++;
      start = 0;
      parity = (a[left] == b[left]) ? 0 : 1;
    }
    if ((parity == 0 && a[left] != b[left]) || (parity == 1 && a[left] == b[left])) {
      cout << "NO" << endl;
      return;
    }
    if (a[left] == '0') count0++;
  ...
  if (parity == 1 && count0 != count1) cout << "NO" << endl;
  else cout << "YES" << endl;
}
