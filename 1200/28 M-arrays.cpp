...
void solve() {
  ...
  frange(i, 0, n) {
    cin >> a[i];
    if (a[i]%m == 0 || (ld)(a[i]%m) == ((ld)(m)) / 2) {
      if (pairs.count(a[i]%m)) pairs[a[i]%m].F++;
      else pairs[a[i]%m] = {1, 0};
    }
    else if (a[i]%m < m-a[i]%m) {
      if (pairs.count(a[i]%m)) pairs[a[i]%m].F++;
      else pairs[a[i]%m] = {1, 0};
    }
    else {
      if (pairs.count(m-a[i]%m)) pairs[m-a[i]%m].S++;
      else pairs[m-a[i]%m] = {0, 1};
    }
  }
