...
void solve() {
  ...
  ll left, right, ans = 0;
  while (n-1-count >= 0) {
    while (n-1-count >= 0 && a[n-1-count] == a[n-1]) count++;
    left = n-1-count;
    right = left+1;

    while (left >= 0 && a[left] != a[n-1]) left--;

    ans += (ll)ceill(log2(((ld)(right-left-1)) / count + 1));
    count *= pow(2, (ll)ceill(log2(((ld)(right-left-1)) / count + 1)));
  }
